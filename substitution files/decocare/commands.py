...
00000000000000
00000000000000
""".strip( ).replace("\n", "")), encoding = 'utf-8') # Changed from bytearray(str("""xxx""".strip( ).replace("\n", "").decode('hex')))
...
class ReadPumpStatus(PumpCommand):
  ...
  def getData(self):
    data = self.data
    normal = { 3: 'normal' } # Changed from "{ 03: 'normal' }"
...
if __name__ == '__main__':
  ...
  if not port or not serial_num:
    print("usage:\n%s <port> <serial>, eg /dev/ttyUSB0 208850" % sys.argv[0]) # Parenthesis added
    sys.exit(1)
  import link
  import stick
  import session
